...
let
  ...
  # wlr-layer (a KDE Plasma repaint fix). niri's xray blur only samples the
  # background-layer backdrop, so on BOTTOM the live wallpaper drops out of it and
  # every transparent window (ghostty, nautilus) renders opaque. PR #585 re-added a
  # `--layer` flag; pin past it and append `--layer background` so the companion
  # `place-within-backdrop` layer-rule in config/niri/rules.kdl folds the wallpaper
  # back into the backdrop. Drop this once nixpkgs ships >= 2026-06-09 (nixpkgs#531461).
  ...
  services.lact = {
    ...
    settings = {
      ...
      profiles = {
        ...
        performance-undervolt.gpus.${rtx5090Id} = {
          power_cap = 525.0;
